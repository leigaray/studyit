...
import com.leiskies.app.enums.Crud;
import com.leiskies.app.util.ConnectionManager;
import com.leiskies.app.util.MethodManager;
import com.leiskies.app.util.QueryManager;

public class ModelDAO<T,PK> extends AbstractModelDAO<T,PK> {
	private static QueryManager queryManager;
	public ModelDAO(Class<T> entityClass) {
		super(entityClass);
		ModelDAO.queryManager = new QueryManager(getEntityClass());
	}
	...
	public Boolean create(List<T> list) {
		...
		try {
			...
			for(T t : list) {
				...
				}	rowsInserted = rowsInserted + preparedStatement.executeUpdate();
			}
			
			if (rowsInserted == list.size()) {
			    System.out.println("A " + getEntityClass().getSimpleName() + " list was registered.");
			    return true;
			}
